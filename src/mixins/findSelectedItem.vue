...
export default Vue.extend({
  methods: {
    findSelectedItem(getElementToEdit) {
      let getWorkplaceData = this.$store.getters.getWorkplaceData;
      let parameters = {
        style: {},
      ...
      parameters.sectionIndex = getWorkplaceData.sections.findIndex(
        (x) => x.id === selectedElement
      );
      ...
      if (getElementToEdit.includes("col")) {
        ...
        parameters.columnIndex = getWorkplaceData.sections[
      ...
      if (getElementToEdit.includes("item")) {
        parameters.itemIndex = getWorkplaceData.sections[
          parameters.sectionIndex
        ].columns[parameters.columnIndex].childs.findIndex(
          (x) => x.id === getElementToEdit
        );
      ...
      switch (parameters.kindOfSelectedItem) {
        case 1:
          parameters.style =
            getWorkplaceData.sections[parameters.sectionIndex].style;
          break;

        case 2:
          parameters.style =
            getWorkplaceData.sections[parameters.sectionIndex].columns[
